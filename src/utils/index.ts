...
export function openRealm(user: Realm.Sync.User, url: string, schema: any[], logger: winston.Logger) {
  ...
  return Realm.open(realmConfiguration).catch((error: any) => {
    logger.error(`Error connecting to the user realm ${url}`);
    logger.error(error);
  });
}


export function readRealm<T>(realm: Realm,
  realmCollectionName: string): Promise<T[] | null> {
